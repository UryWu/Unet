...
def saveResult(save_path,npyfile,flag_multi_class = False,num_class = 2):
    for i,item in enumerate(npyfile):
        ...
        io.imsave(os.path.join(save_path,"%d_predict.png"%i), img_as_uint(img))  # 原来的io.imsave(os.path.join(save_path,"%d_predict.png"%i), img)

def vitualization(history, pic_name):
    # subplot作图参考：https://blog.csdn.net/m0_37362454/article/details/81511427
    ...
    plt.legend()
    plt.savefig(
        './unet_{0}_loss={1:.2f}_accur={2:.2f}_epoch={3}.jpg'.format(pic_name, loss[-1], val_acc[-1],
                                                                                       epochs[-1]))
    # 不在plt.show前保存图片就会使保存的图片空白。':.2f'保留两位小数
    plt.show()
    return val_acc[-1], epochs[-1]
